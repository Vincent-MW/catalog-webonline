...
;(function($){
  // 利用zpeto的animate的动画-css3的动画-easing为了css3的easing(zpeto没有提供easing的扩展)
    ...
    // 初始化函数体，生成对应的DOM节点
		var $coffee = $(this);
		var $audioIco = $coffee.find(".audio_open");
    ...
    $.fn.coffee.stop = function(){
    	console.info("stop");
    	$audioIco.removeClass("css_sprite01").addClass("css_sprite02");
    	bgX = 0;
    	$audioIco.css("background-position-x",bgX+"0px");
...
//      clearInterval(__time_wind);
    ...
    $.fn.coffee.start = function(){
    	console.info("start");
    	$audioIco.removeClass("css_sprite02").addClass("css_sprite01");
    	__time_val = setInterval(function(){
    	  bgX -= 40;
    	  if(bgX<-80){
    		  bgX = 0;
    	  }
    	  $audioIco.css("background-position-x",bgX+"px");
    	}, 100);
//      __time_val = setInterval(function(){
//        steam();
