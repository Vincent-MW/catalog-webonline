...
(function($){
	flux.slider = function(elem, opts) {
		...
		this.options = $.extend({
			...
			delay: 4000,
			pagination: true,
			paginationPointColor:'#9B9B9B',
			paginationPointColorFocus:'#1FB5BF',
			paginationPosition:'right',
			paginationPointShape:'c',
			controls: false,
			captions: false,
			width: null,
			height: null,
			paginationScale:1,//页码点缩放
...
//		});
		 ...
		 var startPoint = {}, endPoint = {}, threshold=20;
        this.element.bind('mousedown touchstart', function(event) {
            if(event.type == 'touchstart'){
                startPoint.left = (event.originalEvent||event).touches[0].pageX;
            } else if(event.type == 'mousedown') {
                startPoint.left = event.pageX;
            }
            event.stopPropagation();
        }).bind('mouseup touchend', function(event) {
            if(event.type == 'touchend'){
                endPoint.left = (event.originalEvent||event).changedTouches[0].pageX;
            } else if(event.type == 'mouseup') {
                endPoint.left = event.pageX;
            }
            if(endPoint.left - startPoint.left > threshold) {
                _this.prev(null, {direction: 'right'});
            } else if(startPoint.left - endPoint.left > threshold) {
                _this.next(null, {direction: 'left'});
            }
	...
	flux.slider.prototype = {
		...
		finishedLoading: function() {
			...
			if(this.options.pagination)
			{
				...
				this.pagination = $('<ul class="pagination"></ul>').css({
					...
					'transform':'scale('+this.options.paginationScale+')',
					'text-align': this.options.paginationPosition
				});
				...
				$(this.images).each(function(index, image){
					var li = $('<li data-index="'+index+'"></li>').css({
						...
						'height': '16px',
						'cursor': 'pointer',
						'background-color':_this.options.paginationPointColor,
					}).appendTo(_this.pagination);
					// TODO 
					if(_this.options.paginationPointShape=="c"){
						li.css('border-radius','50%');
					}

					if(index == 0)
						li.css({'margin-left':0,'background-color':_this.options.paginationPointColorFocus}).addClass('current');
				});
			...
			if(this.options.controls)
			{
				...
				this.nextButton = $('<a href="#">右</a>').css(css).css3({
					'border-radius': '4px'
				...
				this.prevButton = $('<a href="#">左</a>').css(css).css3({
					'border-radius': '4px'
				}).appendTo(this.surface).bind('click', function(event){
					event.preventDefault();
					_this.prev();
		...
		setupImages: function() {
			...
			{
				this.pagination.find('li.current').removeClass('current').css("background-color",this.options.paginationPointColor);
				$(this.pagination.find('li')[this.currentImageIndex]).addClass('current').css("background-color",this.options.paginationPointColorFocus);
			}
		},
	...
	flux.transitions.blocks = function(fluxslider, opts) {
		return new flux.transition_grid(fluxslider, $.extend({
			...
			execute: function() {
				...
				// Get notified when the last transition has completed
				if(this.maxDelayTile !== undefined){
...
(function($) {
	...
	function init(){
		...
		$(".imgBox").each(function(i){
			if($(this).find(".fluxslider").length>0){
				return;
			}
			var that = this;
			var urls = $(this).attr('imgSrcs');//图片地址
			
			var imgGroupPPSColorFRGB = "rgb(31,181,191)";//焦点颜色
			var imgGroupPPSColorSRGB = "rgb(155,155,155)";//页码颜色
			
			var delay = 4000;
			var imgGroupPPSDelay = $(this).attr('imgGroupPPSDelay');
			if(imgGroupPPSDelay){
				try{
					delay = parseFloat(imgGroupPPSDelay);
				}catch(e){}
			}
			var imgGroupPPSColorF = $(this).attr('imgGroupPPSColorF');
			var imgGroupPPSColorS = $(this).attr('imgGroupPPSColorS');
			if(imgGroupPPSColorF!="null"&&imgGroupPPSColorS!="null"&&typeof(imgGroupPPSColorF)!='undefined'&&typeof(imgGroupPPSColorS)!='undefined'){
				imgGroupPPSColorFRGB = "rgb("+imgGroupPPSColorF+")";
				imgGroupPPSColorSRGB = "rgb("+imgGroupPPSColorS+")";
			}
			var shape = $(this).attr('imgGroupPPSShape');//分页形状
			if(shape=="null"||shape==""){
				shape = "c";
			}
			var showPagination = shape!="none";
			
			var position = $(this).attr('imgGroupPPSPostion');//位置
			switch(position){
			case 'l':
				position = "left";
				break;
			...
			var sliderType = $(this).attr("sliderType");//翻页类型
			if(sliderType==null||sliderType==''||sliderType=='null'){
				sliderType = 'slide';
			}
			
			var urlsArray = urls.split(",");
			$.each(urlsArray,function(i,url){
				var img = $("<img src=\""+url+"\"/>");
				img.css("position","absolute");
				img.css("background-color","#FFF");
				$(that).append(img);
			});
			var boxW = $(this).css("width").replace("px","");
			var boxH = $(this).css("height").replace("px","");
			
			var paginationScale = 1;
			if(typeof $(this).attr("paginationScale") !== 'undefined'){
				paginationScale = $(this).attr("paginationScale");
			}
			...
			var options = {
					...
					autoplay: true,
					delay:delay,
					transitions:[sliderType],
					paginationPointColor:imgGroupPPSColorSRGB,
					paginationPointColorFocus:imgGroupPPSColorFRGB,
					paginationPosition:position,
					paginationPointShape:shape,
					paginationScale:paginationScale,
					width:boxW,
					height:boxH
			};
			var fluxID = i;
			$(that).attr("fluxID",fluxID);
			allSliders[fluxID] =	new flux.slider(this, options);
		});
	}
	$(window).load(function(){
		//重置轮播组件//
		var resetFlux = function(curPageArray){
			$(".imgBox").each(function(){
				var that = this;
				var curFluxObj = allSliders[$(this).attr("fluxID")];
				var hasInclude = false;
				$.each(curPageArray,function(){
					if(this.find(that).size()>0){
						hasInclude = true;
					}
				});
				if(hasInclude){
		...
		var resetFlux_swiper = function(){
			$(".imgBox").each(function(){
				var curFluxObj = allSliders[$(this).attr("fluxID")];
				if($(this).closest(".swiper-slide-active").size()>0){
					curFluxObj.start();
				}else{
					curFluxObj.reset();
				}
		...
		if(typeof(car2) !== 'undefined'){//上下翻页，自定义翻页，翻书效果
			if(typeof(car2._on)!=='undefined'){
				car2._on("turnEnd",function(){
					console.info(arguments);
					if(arguments.length==0){
						resetFlux([car2._page.eq(car2._pageNow)]);
					}else{
						resetFlux(arguments[0]);
					}
				});
				car2._on('loadComplete',function(){
					...
					setTimeout(function(){
						if(args.length==0){
							if(typeof car2._page !== 'undefined'){
								resetFlux([car2._page.eq(car2._pageNow)]);
							}
						}else{
							resetFlux(args[0]);
				...
				car2._on('mainConResize',function(){//容器更改尺寸
//					if(arguments.length>0){
//						console.info($(".fluxslider .pagination").length);
//						$(".fluxslider .pagination").css({"transform-origin":"right bottom","transform":"scale("+1/arguments[0]+")"});
//					}
				});
			}
		}else if(typeof(uniSwiper) !== 'undefined'){//左右翻页，幻灯片翻页
			uniSwiper._on("success",function(){
				resetFlux_swiper();
			});
			uniSwiper._on('loadComplete',function(){
				init();
				resetFlux_swiper();
			});
		}
