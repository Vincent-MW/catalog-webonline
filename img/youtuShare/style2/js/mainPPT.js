var uniSwiper={
			...
			*/
			initSwiper:function(){
				...
				var turnModel = "horizontal";
				
				var plantForm = navigator.platform;
				$(".toolTip").remove();
				if (/Win(\d+)/.test(plantForm)||/Mac*/.test(plantForm)){//windows,mac
					_isPC = true;
					turnModel = "vertical";
				}else{
					_isPC = false;
					turnModel = "horizontal";
				}
				
				uniSwiper.initViewPoint();
...
//						virtualTranslate : true, 
						...
						onInit:function(s){
							...
							var slideSize = s.slides.length;
							if(uniSwiper.isLoop){
								slideSize=slideSize-2;
							}
							$("#countPageNum").html(slideSize);
						},
						onSlideChangeEnd:function(s){
							...
							if(uniSwiper.isLoop){
								if(curIndex==0){
									curIndex = s.slides.length-2;
								}else if(curIndex>s.slides.length-2){
									curIndex = 1;
								}
							}else{
								curIndex++;
							}
							$("#curPageNum").html(curIndex);
							uniSwiper._handleEvent("success");
						}
					};
				
				//开启自动翻页
				if($("#isAutoTurnPage").val()=="true"){
					var autoTurnPageGap = $("#autoTurnPageGap").val()*1000;
					initSwiperPara["autoplay"] = autoTurnPageGap;
				}
				switch(uniSwiper._turnMode){
					case "0":
						initSwiperPara["effect"] = "cube";
						initSwiperPara["cube"] = {slideShadows: false,shadow: false,shadowOffset:0,shadowScale:1};
						break;
					case "1":
						initSwiperPara["effect"] = "fade";
						initSwiperPara["fade"] = {crossFade: false};
						break;
					case "2":
						initSwiperPara["effect"] = "coverflow";
						initSwiperPara["slidesPerView"] = 1;
						initSwiperPara["centeredSlides"] = true;
						initSwiperPara["coverflow"] = {rotate: 40,stretch: 60, depth: 0,modifier: 2,slideShadows : false };
						break;
					case "3":
						initSwiperPara["watchSlidesProgress"] = true;
						initSwiperPara["onProgress"] = function(swiper, progress){for (var i = 0; i < swiper.slides.length; i++){var slide = swiper.slides[i];es = slide.style;es.webkitTransform = es.transform ='rotate('+180*slide.progress+'deg)'+ ' '+'translateY('+3200*slide.progress+'px)'; }  };
						break;
					default:
						initSwiperPara["effect"] = "slide";
				}
				uniSwiper.curSwiper =  new Swiper ('.swiper-container', initSwiperPara);
		...
		initViewPoint:function(){
			...
			if(viewport){
				viewport.setAttribute('content', 'width='+viewPointW+', user-scalable=no');
			}
		},
		
		changeArrowState:function(){
			if(uniSwiper.curSwiper){
				var canTurnPage = $(uniSwiper.curSwiper.slides[uniSwiper.curSwiper.activeIndex]).attr("canTurnPage");
				if(canTurnPage=="false"){
					$(".arrowNew").hide();
					uniSwiper.curSwiper.lockSwipes();
				}else{
					$(".arrowNew").show();
					uniSwiper.curSwiper.unlockSwipes();
				}
			}
		},
		//初始化应用位置大小
		initAppSize:function(){
			try{
				...
				uniSwiper.initSpritePostion();
				uniSwiper.initMaskPostion();
			}catch(e){}
		},
		media_init:function(){
...
//					
					...
					//开启自动翻页
					if($("#isAutoTurnPage").val()=="true"){
						uniSwiper.curSwiper.startAutoplay();
					}
	...
	// 				//显示蒙版
					...
					for(var i=uniSwiper.preLoadPageCount;i<pageCount;i++){
						if(uniSwiper.isLoop){
							lastPageArray.push($(uniSwiper.curSwiper.slides[i+1]));
						}else{
							lastPageArray.push($(uniSwiper.curSwiper.slides[i]));
						}
					}
		...
		pageTurnTo:function(turnPageID){
			...
			for(var i=0;i<pageCount;i++){
				var curSlide = $(uniSwiper.curSwiper.slides[i]);
				if(curSlide.attr("pageid")==turnPageID){
					var turnIndex = curSlide.index();
					uniSwiper.curSwiper.slideTo(turnIndex, 500, true);
					break;
				}
			}
		},
		 ...
		 */	
		 loadStartImg:function(startPageIndex,callBack){
			 ...
			 for(var i=0;i<startPageIndex;i++){
				pageArray.push($(uniSwiper.curSwiper.slides[i]));
			 }
		...
		startFirstPageEffect:function(){
		 ...
		 startEffect:function(){
		 		//暂停所有非当前页动画
			 	var otherPages = $(".swiper-slide").not($(uniSwiper.curSwiper.slides[uniSwiper.curSwiper.activeIndex]));
		 		otherPages.find(".animation").each(function(){
		 			$.fn.stopEffect(this);
		 		});
		 		otherPages.find("video,audio") .each(function(){//暂停所有非当前页的音视频
		 			this.pause();
		 			this.currentTime = 0;
		 		});
		 		//启动所有当前页动画
		 		$(uniSwiper.curSwiper.slides[uniSwiper.curSwiper.activeIndex]).find(".animation").each(function(){
		 			//判断是否有控制该元素显示的元素
...
//	 					});
	 	...
	 	// 声音播放
	 	 	audio_play : function(){
	 	 		...
	 	 		var pageAudio;
	 	 		if(curPageEle.find(".pageCon").length>0){
	 	 			pageAudio = $(curPageEle.find(".pageCon")[0]).children("audio");
	 	 		}else{
	 	 			pageAudio = curPageEle.children("audio");
	 	 		}
	 	 		if(pageAudio.length>0){
	 		 		//启动当前页的背景音乐
	 	 			pageAudio.each(function(){
	 		 			this.play();
	 		 		});
	 					...
	 					initSpritePostion:function(){
	 						$(".item").each(function(){
	 							...
	 							if(curAnimationName.indexOf("graduallyIn")>-1){
	 								...
	 								$(this).find(".uniImg").css("width",originW+"px");
	 								$(this).find(".uniImg").css("height",originH+"px");
	 							}
	 						});
	 					},
	 					//初始化背景图片位置
	 					initConPostion:function(){
	 						uniSwiper.curSwiper.update();
	 						var ww = $(window).width();
	 						var wh = $(window).height();
	 						var pageWidth = $("#imgWidth").val();
	 						var pageHeight = $("#imgHeight").val();
	 						$(".item .uniImg").css("width","100%");
	 						$(".item .uniImg").css("height","100%");
	 						$(".page-con-img").css("width",pageWidth +"px");
	 						$(".page-con-img").css("height",pageHeight+"px");
...
//	 							showAll = ww<wh;
	 							...
	 							isNewVersion = false;showAll = true;
	 							scaleEles = $(".swiper-slide");
	 						}
	 						scaleEles.css("transform-origin","left top");
	 						scaleEles.css("-webkit-transform-origin","left top");
	 						var translate = "";
	 						var scale = 1;
	 						var ws = ww/wh;
	 						var cs = pageWidth/pageHeight;
	 						
	 						if(!showAll){//不留白边显示
	 							$(".swiper-slide").css("overflow","hidden");
		 						if(ws>cs){
		 							scale = ww/pageWidth;
		 							translate = "translateY("+(wh-ww/cs)/2+"px)";
		 						}else{
		 							scale = wh/pageHeight;
		 							translate = "translateX("+(ww-wh*cs)/2+"px)";
		 						}
	 						}else{
		 						if(ws>cs){
		 							scale = wh/pageHeight;
		 							translate = "translateX("+((ww/scale-pageWidth)/2)+"px)";
		 						}else{
		 							scale = ww/pageWidth;
		 							translate = "translateY("+((wh/scale-pageHeight)/2)+"px)";
		 						}
	 						}
	 						scaleEles.css("transform","scale("+scale+") "+translate);
	 						scaleEles.css("-webkit-transform","scale("+scale+") "+translate);
	 					},
	 					//初始化蒙版位置
	 					initMaskPostion:function(){
	 						var imgWidth = $("#imgWidth").val();
	 						var imgHeight = $("#imgHeight").val();
	 						
	 						var bgImgW = Math.round(imgWidth*uniSwiper._imgScale);
	 						var bgImgH = Math.round(imgHeight*uniSwiper._imgScale);
	 						
	 						$("#j-mengban canvas").css("margin-left","0px");
	 						$("#j-mengban canvas").css("width",bgImgW+"px");
	 						$("#j-mengban canvas").css("height",bgImgH+"px");
	 						$("#j-mengban canvas").css("left",uniSwiper._paddingLeft+"px");
	 						$("#j-mengban canvas").css("top",uniSwiper._paddingTop+"px");
	 					},
